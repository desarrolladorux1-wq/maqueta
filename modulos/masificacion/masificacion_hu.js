(function(){
  ...
  const phases=['Planificación','Registro del proyecto','Evaluación GIS','Validación técnica','Validación concesionaria','Programación de obra','Ejecución de red','Liquidación','Monitoreo'];
  function openTrace(){
    const phaseDates=[
    ...
    function renderHistory(i){
      const displayRows=histories[i].map((row,index)=>{
        ...
        return row.slice(0,5).concat(statusCell(row[5],index),`<div class="masif-trace-row-actions"><button type="button" class="icon-action edit" data-trace-edit="${index}" title="Editar" aria-label="Editar"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 20h4l11-11-4-4L4 16v4Zm12.5-16.5 4 4 1-1a1.4 1.4 0 0 0 0-2l-2-2a1.4 1.4 0 0 0-2 0l-1 1Z"/></svg></button><button type="button" class="icon-action danger" data-trace-delete="${index}" title="Eliminar" aria-label="Eliminar"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 3h8l1 2h4v2H3V5h4l1-2Zm-2 6h12l-1 12H7L6 9Zm4 2v7h2v-7h-2Zm4 0v7h2v-7h-2Z"/></svg></button><button type="button" class="icon-action pdf ${row[6]?'uploaded':''}" data-trace-pdf="${index}" title="${row[6]?'Reemplazar PDF':'Subir PDF'}" aria-label="${row[6]?'Reemplazar PDF':'Subir PDF'}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 2h8l5 5v15H6V2Zm8 2.5V8h3.5L14 4.5ZM9 18h2v-4.2l1.6 1.6 1.4-1.4-4-4-4 4 1.4 1.4L9 13.8V18Z"/></svg>${row[6]?'<span class="icon-check" aria-hidden="true">✓</span>':''}</button></div>`);
      });
  ...
  function applyProjectTerminology(){
    const replacements=[
      ['#infoName','PROYECTO SAUNA 1','PROYECTO SAUNA 1'],
      ['#modalTitle','Crear proyecto','Crear proyecto']
    ];
    ...
    if(modal){
      ...
      nodes.forEach(node=>{node.nodeValue=node.nodeValue.replace(/proyectos/gi,'proyectos').replace(/proyecto/gi,'proyecto');});
    }
  }
