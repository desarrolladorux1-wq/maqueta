...
window.__MODULE_PATHS = {
  ...
  'electricidad-source': 'modulos/electricidad_al_toque/electricidad_al_toque.html?v=elec-hogares-sin-servicio-v11',
  ...
  'masificacion-source': 'modulos/masificacion/masificacion.html?v=masif-project-tools-v15',
  'masificacion-satcontrol-source': 'modulos/masificacion/masificacion_satcontrol.html?v=masif-strata-toggle-v18',
  'mcter-source': 'modulos/mtcer/mtcer.html?v=mcter-sin-evidencias-v4'
};
...
async function loadShellModules() {
  ...
  var tasks = [];
  var adminVersion = 'bonogas-field-gis-v2';
...
/* BONOGAS SATCONTROL · manzanas, coordenadas y supervisión de campo. */
(function(){
  let blockLayer=null,blockFeatures=[],coordMode='geo',workPointMode=false,pendingWorkPoint=null,workMarkers=null;
  const works=[
    {id:'SUP-001',title:'Verificación de acometida',supervisor:'María Fernanda Rojas',date:'2026-08-13',priority:'Alta',lat:-16.3893,lng:-71.5446},
    {id:'SUP-002',title:'Control de habilitación',supervisor:'Equipo de Supervisión Norte',date:'2026-08-15',priority:'Media',lat:-16.3989,lng:-71.5350}
  ];
  const esc=v=>String(v==null?'':v).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  function isBonogas(){const main=document.querySelector('.main');return !!(main&&main.classList.contains('bonogas-satcontrol-mode'));}
  function open(id){if(typeof openModal==='function')openModal(id);else document.getElementById(id)?.classList.add('open')}
  function close(id){if(typeof closeModal==='function')closeModal(id);else document.getElementById(id)?.classList.remove('open')}
  function mapReady(){return typeof leafletMap!=='undefined'&&leafletMap&&window.L}
  function currentCenter(){return mapReady()?leafletMap.getCenter():{lat:-16.3989,lng:-71.5350}}
  function clearBlockLayer(){if(blockLayer&&mapReady()){try{leafletMap.removeLayer(blockLayer)}catch(e){}}blockLayer=null;blockFeatures=[]}
  function kmlBlocks(text){const xml=new DOMParser().parseFromString(text,'text/xml'),rows=[];xml.querySelectorAll('Placemark').forEach((mark,index)=>{const name=mark.querySelector('name')?.textContent?.trim()||`Manzana ${String(index+1).padStart(3,'0')}`,coordsText=mark.querySelector('Polygon coordinates')?.textContent||'';const coords=coordsText.trim().split(/\s+/).map(x=>x.split(',').map(Number)).filter(x=>Number.isFinite(x[0])&&Number.isFinite(x[1])).map(x=>[x[1],x[0]]);if(coords.length>=3)rows.push({id:`MZ-${String(index+1).padStart(3,'0')}`,name,coords})});return rows}
  function demoShapeBlocks(){const c=currentCenter(),rows=[];for(let i=0;i<12;i++){const col=i%4,row=Math.floor(i/4),lat=c.lat+(row-1)*.0024,lng=c.lng+(col-1.5)*.0028,w=.00105,h=.00082,j=(i%3)*.00012;rows.push({id:`SHP-${String(i+1).padStart(3,'0')}`,name:`Manzana catastral ${i+1}`,coords:[[lat-h,lng-w],[lat-h*.82,lng+w],[lat+h,lng+w-j],[lat+h*.88,lng-w-j]]})}return rows}
  function drawBlocks(rows,source){if(!mapReady())return;clearBlockLayer();blockFeatures=rows;blockLayer=L.featureGroup();rows.forEach((row,index)=>{const colors=['#0ea5e9','#22c55e','#f59e0b','#8b5cf6'],layer=L.polygon(row.coords,{color:colors[index%colors.length],weight:2.5,fillColor:colors[index%colors.length],fillOpacity:.18}).bindPopup(`<b>${esc(row.name)}</b><br>Código: ${esc(row.id)}<br>Fuente: ${esc(source)}`);row.layer=layer;layer.addTo(blockLayer)});blockLayer.addTo(leafletMap);try{leafletMap.fitBounds(blockLayer.getBounds(),{padding:[40,40],maxZoom:17})}catch(e){}renderBlockResults(blockFeatures);if(typeof showToast==='function')showToast(`${rows.length} manzanas cargadas desde ${source}`)}
  function renderBlockResults(rows){const box=document.getElementById('bonoBlockResults');if(!box)return;box.innerHTML=rows.length?rows.map(row=>`<div class="bonogas-block-result"><span><b>${esc(row.id)}</b><br>${esc(row.name)}</span><button type="button" data-bono-block="${esc(row.id)}">Ubicar</button></div>`).join(''):'<span>No se encontraron manzanas.</span>'}
  function locateBlock(id){const row=blockFeatures.find(x=>x.id===id);if(!row||!row.layer||!mapReady())return;close('bonoBlockSearchModal');leafletMap.fitBounds(row.layer.getBounds(),{padding:[70,70],maxZoom:18});row.layer.openPopup();row.layer.setStyle({weight:5,fillOpacity:.35});setTimeout(()=>row.layer?.setStyle({weight:2.5,fillOpacity:.18}),1600)}
  function geoToUtm(lat,lon){const a=6378137,eccSquared=.00669438,k0=.9996,zone=Math.floor((lon+180)/6)+1,longOrigin=(zone-1)*6-180+3,latRad=lat*Math.PI/180,longRad=lon*Math.PI/180,longOriginRad=longOrigin*Math.PI/180,eccPrimeSquared=eccSquared/(1-eccSquared),N=a/Math.sqrt(1-eccSquared*Math.sin(latRad)**2),T=Math.tan(latRad)**2,C=eccPrimeSquared*Math.cos(latRad)**2,A=Math.cos(latRad)*(longRad-longOriginRad),M=a*((1-eccSquared/4-3*eccSquared**2/64-5*eccSquared**3/256)*latRad-(3*eccSquared/8+3*eccSquared**2/32+45*eccSquared**3/1024)*Math.sin(2*latRad)+(15*eccSquared**2/256+45*eccSquared**3/1024)*Math.sin(4*latRad)-(35*eccSquared**3/3072)*Math.sin(6*latRad));let easting=k0*N*(A+(1-T+C)*A**3/6+(5-18*T+T**2+72*C-58*eccPrimeSquared)*A**5/120)+500000,northing=k0*(M+N*Math.tan(latRad)*(A**2/2+(5-T+9*C+4*C**2)*A**4/24+(61-58*T+T**2+600*C-330*eccPrimeSquared)*A**6/720));if(lat<0)northing+=10000000;return{easting,northing,zone,hemisphere:lat<0?'S':'N'}}
  function utmToGeo(easting,northing,zone,hemisphere){const a=6378137,e=.0818191908,e1=(1-Math.sqrt(1-e*e))/(1+Math.sqrt(1-e*e)),k0=.9996,x=easting-500000,y=hemisphere==='S'?northing-10000000:northing,longOrigin=(zone-1)*6-180+3,eccPrimeSquared=e*e/(1-e*e),M=y/k0,mu=M/(a*(1-e*e/4-3*e**4/64-5*e**6/256)),phi1=mu+(3*e1/2-27*e1**3/32)*Math.sin(2*mu)+(21*e1**2/16-55*e1**4/32)*Math.sin(4*mu)+(151*e1**3/96)*Math.sin(6*mu),N1=a/Math.sqrt(1-e*e*Math.sin(phi1)**2),T1=Math.tan(phi1)**2,C1=eccPrimeSquared*Math.cos(phi1)**2,R1=a*(1-e*e)/Math.pow(1-e*e*Math.sin(phi1)**2,1.5),D=x/(N1*k0),lat=phi1-(N1*Math.tan(phi1)/R1)*(D**2/2-(5+3*T1+10*C1-4*C1**2-9*eccPrimeSquared)*D**4/24+(61+90*T1+298*C1+45*T1**2-252*eccPrimeSquared-3*C1**2)*D**6/720),lon=(D-(1+2*T1+C1)*D**3/6+(5-2*C1+28*T1-3*C1**2+8*eccPrimeSquared+24*T1**2)*D**5/120)/Math.cos(phi1);return{lat:lat*180/Math.PI,lng:longOrigin+lon*180/Math.PI}}
  function convertCoordinates(){const result=document.getElementById('bonoCoordinateResult');if(coordMode==='geo'){const lat=Number(document.getElementById('bonoCoordLat')?.value),lng=Number(document.getElementById('bonoCoordLng')?.value);if(!Number.isFinite(lat)||!Number.isFinite(lng)){result.textContent='Ingrese latitud y longitud válidas.';return}const u=geoToUtm(lat,lng);result.innerHTML=`<b>UTM WGS84 Zona ${u.zone}${u.hemisphere}</b><br>Este: ${u.easting.toFixed(2)} m · Norte: ${u.northing.toFixed(2)} m`;if(mapReady())leafletMap.setView([lat,lng],17)}else{const east=Number(document.getElementById('bonoCoordEast')?.value),north=Number(document.getElementById('bonoCoordNorth')?.value),zone=Number(document.getElementById('bonoCoordZone')?.value),hem=document.getElementById('bonoCoordHemisphere')?.value||'S';if(!Number.isFinite(east)||!Number.isFinite(north)||!Number.isFinite(zone)){result.textContent='Ingrese coordenadas UTM válidas.';return}const g=utmToGeo(east,north,zone,hem);result.innerHTML=`<b>Coordenadas geográficas WGS84</b><br>Latitud: ${g.lat.toFixed(7)} · Longitud: ${g.lng.toFixed(7)}`;if(mapReady())leafletMap.setView([g.lat,g.lng],17)}}
  function renderWorks(){const box=document.getElementById('bonoWorkList');if(!box)return;box.innerHTML=works.map(w=>`<div class="bonogas-work-item"><span><b>${esc(w.id)} · ${esc(w.title)}</b><br>${esc(w.supervisor)} · ${esc(w.date)} · ${esc(w.priority)}</span><button type="button" data-bono-work="${esc(w.id)}">Ver punto</button></div>`).join('');drawWorkMarkers()}
  function drawWorkMarkers(){if(!mapReady())return;if(workMarkers){try{leafletMap.removeLayer(workMarkers)}catch(e){}}workMarkers=L.layerGroup();works.forEach(w=>{const icon=L.divIcon({className:'',html:'<div class="bonogas-supervision-pin"><span>✓</span></div>',iconSize:[34,34],iconAnchor:[17,34]});L.marker([w.lat,w.lng],{icon}).bindPopup(`<b>${esc(w.title)}</b><br>${esc(w.id)} · ${esc(w.date)}<br>Supervisor: ${esc(w.supervisor)}<br>Prioridad: ${esc(w.priority)}`).addTo(workMarkers)});if(isBonogas())workMarkers.addTo(leafletMap)}
  function pickWorkPoint(){if(!mapReady()){showToast('El mapa todavía está cargando');return}close('bonoFieldWorkModal');workPointMode=true;document.querySelector('.main')?.classList.add('bonogas-pick-work-point');showToast('Haga clic en el mapa para asignar el punto de supervisión');leafletMap.once('click',e=>{if(!workPointMode)return;pendingWorkPoint=e.latlng;workPointMode=false;document.querySelector('.main')?.classList.remove('bonogas-pick-work-point');document.getElementById('bonoWorkLat').value=e.latlng.lat.toFixed(7);document.getElementById('bonoWorkLng').value=e.latlng.lng.toFixed(7);open('bonoFieldWorkModal')})}
  function saveWork(){const lat=Number(document.getElementById('bonoWorkLat')?.value),lng=Number(document.getElementById('bonoWorkLng')?.value);if(!Number.isFinite(lat)||!Number.isFinite(lng)){showToast('Asigne primero un punto en el mapa');return}works.unshift({id:`SUP-${String(works.length+1).padStart(3,'0')}`,title:document.getElementById('bonoWorkTitle')?.value||'Supervisión de campo',supervisor:document.getElementById('bonoWorkSupervisor')?.value||'-',date:document.getElementById('bonoWorkDate')?.value||'',priority:document.getElementById('bonoWorkPriority')?.value||'Media',lat,lng});pendingWorkPoint=null;renderWorks();showToast('Trabajo de supervisión programado')}
  document.addEventListener('change',async e=>{if(e.target.id!=='bonoBlockFileInput')return;const files=[...e.target.files],status=document.getElementById('bonoBlockFileStatus');if(!files.length)return;try{const kml=files.find(f=>/\.kml$/i.test(f.name));let rows;if(kml){rows=kmlBlocks(await kml.text());if(!rows.length)throw new Error('El KML no contiene polígonos')}else rows=demoShapeBlocks();drawBlocks(rows,kml?'KML':'Shapefile');status.textContent=`${rows.length} manzanas cargadas · ${files.map(f=>f.name).join(', ')}`}catch(err){status.textContent='No se pudo leer la capa: '+err.message;showToast(status.textContent)}});
  document.addEventListener('click',e=>{if(e.target.closest('#bonoBlockSearchBtn')){open('bonoBlockSearchModal');renderBlockResults(blockFeatures)}if(e.target.closest('#bonoFieldWorkBtn')){renderWorks();open('bonoFieldWorkModal')}const tab=e.target.closest('[data-bono-coord-mode]');if(tab){coordMode=tab.dataset.bonoCoordMode;document.querySelectorAll('[data-bono-coord-mode]').forEach(x=>x.classList.toggle('active',x===tab));document.getElementById('bonoGeoFields').hidden=coordMode!=='geo';document.getElementById('bonoUtmFields').hidden=coordMode!=='utm'}if(e.target.closest('#bonoConvertCoordinatesBtn'))convertCoordinates();if(e.target.closest('#bonoBlockSearchAction')){const q=(document.getElementById('bonoBlockQuery')?.value||'').trim().toLowerCase();renderBlockResults(blockFeatures.filter(x=>!q||`${x.id} ${x.name}`.toLowerCase().includes(q)))}const block=e.target.closest('[data-bono-block]');if(block)locateBlock(block.dataset.bonoBlock);if(e.target.closest('#bonoPickWorkPointBtn'))pickWorkPoint();if(e.target.closest('#bonoSaveWorkBtn'))saveWork();const work=e.target.closest('[data-bono-work]');if(work){const w=works.find(x=>x.id===work.dataset.bonoWork);if(w&&mapReady()){close('bonoFieldWorkModal');leafletMap.setView([w.lat,w.lng],17);setTimeout(()=>{workMarkers?.eachLayer(layer=>{const ll=layer.getLatLng?.();if(ll&&Math.abs(ll.lat-w.lat)<1e-7&&Math.abs(ll.lng-w.lng)<1e-7)layer.openPopup()})},200)}}});
  document.addEventListener('keydown',e=>{if(e.key!=='Escape')return;workPointMode=false;pendingWorkPoint=null;document.querySelector('.main')?.classList.remove('bonogas-pick-work-point');['bonoBlockSearchModal','bonoFieldWorkModal'].forEach(close)});
  window.ensureBonogasFieldTools=function(){renderWorks()};
})();
(function(){
  function setStep(n){
...
/* MASIFICACION · versiones de ingenieria, alcance y aptitud tecnica. */
(function(){
  const versionStore=new Map(),checklistStore=new Map();
  ...
  function projectName(){const p=project();return p?(p.nombre||p.id||'Proyecto seleccionado'):'Proyecto seleccionado';}
  function hydrate(){
    const p=project(),key=p?.id||'default',state=versionStore.get(key)||{progress:76,phase:'Ingeniería de detalle'};
    const range=document.getElementById('masifRealProgressRange'),phase=document.getElementById('masifPlanningPhase');
    if(range)range.value=state.progress;if(phase)phase.value=state.phase;
    updateProgress(state.progress);
    const name=document.getElementById('masifChecklistProject');if(name)name.value=projectName()+(p?.id?' · '+p.id:'');
    const saved=checklistStore.get(key);if(saved){
      const result=document.getElementById('masifChecklistResult'),notes=document.getElementById('masifChecklistNotes');
      if(result)result.value=saved.result;if(notes)notes.value=saved.notes;
      document.querySelectorAll('#masifTechnicalChecks input').forEach((x,i)=>x.checked=!!saved.checks[i]);
    }
  }
  function updateProgress(value){
    const n=Math.max(0,Math.min(100,Number(value)||0)),gap=n-100;
    const real=document.getElementById('masifRealProgressValue'),gapEl=document.getElementById('masifProgressGapValue'),label=document.getElementById('masifCompareLabel'),bar=document.getElementById('masifScopeRealBar');
    if(real)real.textContent=n+'%';if(gapEl)gapEl.textContent=(gap>0?'+':'')+gap+'%';if(label)label.textContent=n+'% ejecutado';if(bar)bar.style.width=n+'%';
    const km=(4.82*n/100).toFixed(2),lots=Math.round(128*n/100),valves=Math.round(14*n/100);
    const kmEl=document.getElementById('masifBuiltKm'),lotsEl=document.getElementById('masifBuiltLots'),valvesEl=document.getElementById('masifBuiltValves');
    if(kmEl)kmEl.textContent=km+' km';if(lotsEl)lotsEl.textContent=lots;if(valvesEl)valvesEl.textContent=valves;
  }
  function openVersions(){hydrate();modal('masifVersionControlModal',true);}
  function openChecklist(){hydrate();modal('masifLandChecklistModal',true);}
  document.getElementById('masifVersionControlBtn')?.addEventListener('click',openVersions);
  document.getElementById('masifLandChecklistBtn')?.addEventListener('click',openChecklist);
  document.getElementById('masifRealProgressRange')?.addEventListener('input',e=>updateProgress(e.target.value));
  document.getElementById('masifSaveVersionsBtn')?.addEventListener('click',()=>{
    const p=project(),key=p?.id||'default',progress=Number(document.getElementById('masifRealProgressRange')?.value||0),phase=document.getElementById('masifPlanningPhase')?.value||'Ingeniería básica';
    versionStore.set(key,{progress,phase,planningFile:document.getElementById('masifPlanningFile')?.files?.[0]?.name||'',builtFile:document.getElementById('masifBuiltFile')?.files?.[0]?.name||''});
    document.getElementById('masifEngineeringPhaseValue').textContent=phase;
    if(typeof showToast==='function')showToast('Versiones Anteproyecto y As-built guardadas por separado');
  });
  document.getElementById('masifOpenMapComparisonBtn')?.addEventListener('click',()=>{
    modal('masifVersionControlModal',false);
    const frame=document.getElementById('masificacionFrame');
    try{if(frame?.contentWindow?.openCompareModal){frame.contentWindow.openCompareModal();return;}}catch(e){}
    if(typeof showToast==='function')showToast('Comparador cartográfico preparado');
  });
  document.getElementById('masifUseMapLocationBtn')?.addEventListener('click',()=>{
    let center=null;try{center=document.getElementById('masificacionFrame')?.contentWindow?.leafletMap?.getCenter?.();}catch(e){}
    if(!center){const p=project();center={lat:p?.lat||-16.39889,lng:p?.lng||-71.535};}
    document.getElementById('masifChecklistLat').value=Number(center.lat).toFixed(6);document.getElementById('masifChecklistLng').value=Number(center.lng).toFixed(6);
    if(typeof showToast==='function')showToast('Coordenadas tomadas del mapa');
  });
  document.getElementById('masifChecklistPhotos')?.addEventListener('change',e=>{
    const list=document.getElementById('masifChecklistPhotoList'),lat=document.getElementById('masifChecklistLat')?.value,lng=document.getElementById('masifChecklistLng')?.value;
    if(!list)return;Array.from(e.target.files||[]).forEach(file=>{const row=document.createElement('article');row.innerHTML='<b></b><span></span>';row.querySelector('b').textContent=file.name;row.querySelector('span').textContent=lat+', '+lng;list.appendChild(row);});
  });
  document.getElementById('masifSaveChecklistBtn')?.addEventListener('click',()=>{
    const p=project(),key=p?.id||'default',checks=Array.from(document.querySelectorAll('#masifTechnicalChecks input')).map(x=>x.checked);
    checklistStore.set(key,{checks,result:document.getElementById('masifChecklistResult')?.value,notes:document.getElementById('masifChecklistNotes')?.value,lat:document.getElementById('masifChecklistLat')?.value,lng:document.getElementById('masifChecklistLng')?.value,photos:document.getElementById('masifChecklistPhotoList')?.children.length||0});
    modal('masifLandChecklistModal',false);if(typeof showToast==='function')showToast('Evaluación técnica y fotografías georreferenciadas guardadas');
  });
  document.addEventListener('keydown',e=>{if(e.key==='Escape'){modal('masifVersionControlModal',false);modal('masifLandChecklistModal',false);}});
})();
